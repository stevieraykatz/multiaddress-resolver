...
abstract contract MultiaddressResolver is IMultiaddressResolver, ResolverBase {
    ...
    mapping(
        uint64 version => mapping(bytes32 node => mapping(uint256 cointype => EnumerableSetLib.AddressSet addressSet))
    ) versionable_addressSets;
    ...
    function removeAddrs(bytes32 node, address[] calldata addresses, uint256 cointype)
        public
        virtual
        authorised(node)
    {
    ...
    function addrs(bytes32 node, uint256 cointype) external view virtual returns (address[] memory) {
        return versionable_addressSets[recordVersions[node]][node][cointype].values();
    }

    function addrs(bytes32 node) external view virtual returns (address[] memory) {
        return versionable_addressSets[recordVersions[node]][node][COIN_TYPE_ETH].values();
    }

    function supportsInterface(bytes4 interfaceID) public view virtual override returns (bool) {
        return interfaceID == type(IMultiaddressResolver).interfaceId || super.supportsInterface(interfaceID);
    }
}
